...
14. **Tray icon — cache BYTES, not `Icon` instances.** The tray ships a single embedded `.ico` (`assets/Speedtest.ico`, included as a WPF `<Resource Link="Resources/AppIcon.ico"/>`). `IconFactory.LoadAppIconBytes()` reads it via `Application.GetResourceStream("pack://application:,,,/Resources/AppIcon.ico")` and returns `byte[]`; `IconFactory.FromBytes` mints a **fresh `Icon` for every assignment** to `_trayIcon.Icon`. State (Browsing/Blocked/Paused/Disabled) is conveyed by the tooltip text only — same icon for every state. Reason for the bytes-not-instances policy: H.NotifyIcon's `Icon` dependency-property setter disposes the previously-assigned `Icon` when a new one is set — so caching `Icon` instances and re-assigning them across state transitions throws `ObjectDisposedException` on the next `Icon.get_Handle()` (we hit this in WDAGUtilityAccount sandbox testing on 2026-05-24, observed as a `System.ObjectDisposedException` from `H.NotifyIcon.TaskbarIcon.UpdateIcon` in the Application event log). Don't "optimise" this back to cached `Icon` fields, and don't try `Icon.FromHandle` directly (handle ownership semantics burn you on dispose). The `OnExit` cleanup deliberately does NOT dispose the icons — H.NotifyIcon owns them.
...
16. **Watchdog supervision is folded into `Worker.TickOnceAsync`, *and* called once eagerly from `Worker.StartAsync`.** No separate timer. The eager call at startup is what closes the post-install gap: the MSI starts the service before any tick has run, and without the eager call the user sees an empty tray for up to a full `TickInterval` after install. Each tick `SuperviseWatchdog()` checks `_watchdogPid`: if dead and we're not Disabled and `WatchdogInterval` has elapsed since the last attempt and an interactive user exists and the watchdog exe lives at `Path.Combine(AppContext.BaseDirectory, ServiceOptions.WatchdogExeName)`, it calls `ISessionLauncher.Launch` and stores the new PID. `Worker.StopAsync` does NOT kill the watchdog — it can decide to exit on its own when the pipe ping fails repeatedly (currently it just logs).

17. **`SessionLauncher` uses `DllImport`, not `LibraryImport`.** A previous attempt with LibraryImport failed because LibraryImport's source generator emits unsafe code, which collides with `TreatWarningsAsErrors=true` unless `<AllowUnsafeBlocks>true</AllowUnsafeBlocks>` is set. DllImport is safe-code-only and fully supported on .NET 10. If you ever revisit this, do NOT swap to LibraryImport without setting `<AllowUnsafeBlocks>` only on that single project AND verifying the build stays clean. **Session discovery is two-tier**: `TryFindInteractiveSession` first probes `WTSGetActiveConsoleSessionId` (fast path for the local console user) and skips it if it's `INVALID_SESSION_ID`, session 0 (Services session — non-interactive), or has no user token. On miss it falls back to `WTSEnumerateSessionsW` and picks the first `WTSActive` session with a real user token. The fallback is what makes the post-install spawn reliable on RDP and on the brief window after Windows boot when the console session id is not yet populated. Don't "simplify" this back to a single `WTSGetActiveConsoleSessionId` call — that's the regression we just fixed.

18. **Watchdog is a per-user/per-session WinExe (no console window).** `OutputType` is `WinExe`, not `Exe` — so Windows does not allocate a console for it. The code still uses `Console.WriteLine` for diagnostic logs (Windows redirects those to NUL when no console is attached, which is fine). If you need to see logs during development, attach a debugger and watch `Debug.WriteLine` output, or run the watchdog standalone from a terminal. Single-instance via `Local\FocusGuard.Watchdog` mutex (the `Local\` prefix scopes the mutex to the session, which matches what the Service does — one watchdog per active console session). It pings `focusguard.cmd` over the named pipe every 5s with `GetStatus`, and respawns `FocusGuard.Tray.exe` from `AppContext.BaseDirectory` if no tray process is found by name. It does not kill itself if the pipe ping fails — the Service can survive transient stalls and we don't want the watchdog to flap. **Don't switch back to `OutputType=Exe`** — that re-introduces the console window flicker that the user explicitly rejected. Symptom: a black "C:\Program Files\FocusGuard\FocusGuard.Watchdog.exe" window appears on screen and pops back if closed (because `SuperviseWatchdog` immediately respawns). Related: `SessionLauncher.Launch` uses `CREATE_NO_WINDOW` (not `CREATE_NEW_CONSOLE`) so even if a child exe is mistakenly authored as Console, no window appears.
