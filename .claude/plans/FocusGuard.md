...
9. [x] `FocusGuard.Watchdog` + service-side `SessionLauncher`. *(done — `SessionLauncher.cs` uses `DllImport` (`WTSGetActiveConsoleSessionId` / `WTSQueryUserToken` / `DuplicateTokenEx` / `CreateEnvironmentBlock` / `CreateProcessAsUserW`) to spawn into the active console session; `NoOpSessionLauncher` for tests/non-Windows. Worker tick supervises a per-process watchdog PID, throttled by `ServiceOptions.WatchdogInterval` (default 10s). `FocusGuard.Watchdog` is a single-instance console (`Local\FocusGuard.Watchdog` mutex) that heartbeats `GetStatus` every 5s and respawns `FocusGuard.Tray.exe` if missing. 8 new tests — 4 SessionLauncher smoke (Windows-only), 4 Worker watchdog wiring.)*
10. [x] Tamper tests pass. *(done — Worker now emits `AuditCategory.Tamper` on every clock-backward jump and `AuditCategory.StateTransition` on every state change; `tests/FocusGuard.Service.Tests/TamperTests.cs` covers the four end-to-end contracts: clock backward during Browsing ends the session and writes both audit entries, clock backward while Blocked writes only the Tamper entry, AdminPause/EndPause and Disable/Enable each emit StateTransition lines. `FakeClock.AdvanceWallOnly` was added to drive the wall/monotonic divergence. Service-stop ACL hardening + ProgramData ACL are install-time concerns covered by the WiX `sc sdset` and `Util:PermissionEx` in step 11; watchdog respawn is covered by step 9's WorkerWatchdogTests.)*
11. [x] WiX installer — service install, ACLs, sc-failure config, sc-sdset, uninstall guard. *(done — `src/FocusGuard.Installer/FocusGuard.Installer.wixproj` (WixToolset.Sdk 5.0.2, Util.wixext 5.0.2) + `Product.wxs`. ServiceInstall registers FocusGuard as LocalSystem auto-start; deferred custom actions configure failure auto-restart (5s ×3) and sdset the service with an SDDL that removes SERVICE_STOP from BUILTIN\Administrators; HKLM Run-keys auto-launch Tray + Watchdog at logon; `Util:PermissionEx` on `%ProgramData%\FocusGuard` grants only SYSTEM (full) and Administrators (read), with Users implicitly denied by replacing the inherited DACL. Build: `dotnet build src/FocusGuard.Installer/...wixproj -c Release` produces `FocusGuard.msi` cleanly. Uninstall password guard NOT yet implemented — explicit TODO in `Product.wxs` with the hook point and pseudocode; documented in VERIFICATION.md.)*
12. [x] Manual VM verification per section above. *(done — runbook at `VERIFICATION.md` covers install, default block posture, whitelist round-trip, split-session budget, reboot persistence, tamper checks, admin flows, 7am rollover, uninstall, audit-log spot-check, and out-of-scope bypasses. The runbook is the human-driven gate before shipping.)*
